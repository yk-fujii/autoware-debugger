...
public:
  ...
  void init()
  {
    ...
    //    Subs_["lane_waypoints_array"] = nh_.subscribe("lane_waypoints_array", 10, laneArrayCallback);
    //    Subs_["closest_waypoint"] = nh_.subscribe("closest_waypoint", 1, closestWaypointCallback);
    Subs_["current_pose"] = nh_.subscribe("current_pose", 1, &LaneDebugger::currentPoseCallback, this);
    Subs_["clicked_point"] = nh_.subscribe("/clicked_point", 10, &LaneDebugger::clickedPointCallback, this);

    Pubs_["lane_debug"] = nh_.advertise<visualization_msgs::MarkerArray>("lane_debug", 9, true);
  }
  void run()
  {
    init();
